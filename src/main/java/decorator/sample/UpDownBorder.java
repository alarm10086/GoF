package decorator.sample;

public class UpDownBorder extends Border {
    private final char borderChar;                         // 表示装饰边框的字符

    public UpDownBorder(final Display display, final char ch) {  // 通过构造函数指定Display和装饰边框字符
        super(display);
        this.borderChar = ch;
    }

    @Override
    public int getColumns() {                        // 列数与要显示的内容的字符数相同
        return display.getColumns();
    }

    @Override
    public int getRows() {                           // 行数是内容的行数加上上下边框
        return 1 + display.getRows() + 1;
    }

    @Override
    public String getRowText(final int row) {              // 获取指定行的内容
        if (row == 0 || row == getRows() - 1) {
            return makeLine(borderChar, getColumns());
        } else {
            return display.getRowText(row - 1);
        }
    }

    private String makeLine(final char ch, final int count) {    // 生成一个由count个字符ch连续组成的字符串
        final StringBuffer buf = new StringBuffer();
        for (int i = 0; i < count; i++) {
            buf.append(ch);
        }
        return buf.toString();
    }
}
